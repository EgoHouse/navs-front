...
import { CatalogService } from '@modules/catalog/api/catalogApi';
...
const MenuManagement: React.FC<MenuManagementProps> = ({}) => {
  ...
  useEffect(() => {
    ...
    if (categoriesError) {
      setError(categoriesError.message || 'Error al cargar las categorías');
    }
  }, [categoriesError]);
  ...
  const getAvailableSubcategories = (categoryId: string): string[] => {
    ...
    return category.subcategories.map((sub: MenuSubcategory) => sub.name);
  };
  ...
  const getAvailableSubsections = (
    ...
    const subcategory = category.subcategories.find(
      (sub: MenuSubcategory) => sub.name === subcategoryName
    );
    if (!subcategory || !subcategory.subsections) return [];

    return subcategory.subsections.map((subsec: MenuSubsection) => subsec.name);
  };
  ...
  const saveItemHandler = async () => {
    ...
    try {
      ...
      const itemData: AddItemRequest = {
        ...
        ...(itemForm.subsectionName && { subsectionName: itemForm.subsectionName }),
        item: {
          name: itemForm.name,
  ...
  const saveEditItemHandler = async () => {
    ...
    try {
      ...
      const updateData: UpdateItemRequest = {
        ...
        ...(editingItem.subsectionName && { subsectionName: editingItem.subsectionName }),
        ...
        {modalType === 'editItem' && (
          <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative w-full max-w-4xl bg-gray-900 rounded-xl border border-gray-700 p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
              onClick={(e: React.MouseEvent) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-4">
                <div>
      ...
      <div className="grid gap-6">
        {categories?.map((category: Category) => (
          ...
          >
            ...
            <div className="space-y-4">
              {category.subcategories?.map((subcategory: MenuSubcategory) => (
                <div
                ...
                >
                  ...
                  {subcategory.subsections?.map((subsection: MenuSubsection) => (
                    <div
                      key={subsection.name}
